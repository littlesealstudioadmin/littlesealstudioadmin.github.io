  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 select-none">
      {recentProjects.map((item) => {
        ...
        return (
          <a key={item.id} href={`/projects/${item.id}`} className="group block bg-muted border border-border overflow-hidden hover:border-secondary-foreground/30 relative hover:bg-background active:scale-100 hover:scale-102 animation">
            {firstImage && (
