...
export function ProjectCard({ item }: ProjectCardProps) {
  ...
  return (
    <a href={`/projects/${item.id}`} className="group block bg-muted border border-border/50 overflow-hidden hover:border-foreground/20 relative hover:bg-card active:scale-100 hover:scale-102 animation select-none">
      {firstImage && (
      ...
      <div className="p-3 bg-muted group-hover:bg-card animation z-10 group-hover:-translate-y-14 h-20 translate-y-0">
        <div className="flex justify-between items-center opacity-100 group-hover:opacity-0 animation">
          <span className="text-xs uppercase tracking-widest text-muted-foreground">{item.data.category}</span>
          <span className="text-xs text-muted-foreground font-mono">
            {item.data.startDate && (
              <>
                {item.data.startDate} ~ {item.data.inProgress ? "진행중" : item.data.endDate}
              </>
            )}
          </span>
        </div>
